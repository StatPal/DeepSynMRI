...
for target in range(1, 3):
	...
	#### Original images:
	#dat_target_orig = dat_orig[:,:,:,target]
	# scaling_factor = np.amax(dat_target)
	#dat_target_orig = dat_target_orig*255/scaling_factor   ## Check this

	#ar_orig = np.clip(dat_target_orig,0,255).astype(np.uint8)
	#if dat_target_orig.shape[0] == 1:
	#	ar_orig = ar_orig[0]
	#else:
	#	ar_orig = ar_orig.transpose(0, 1, 2)
	#act_image_orig = ar_orig


	## Pad images:
	#img_pil = np.zeros(new_shape)
	#img_pil[0:256,0:256,0:20] = act_image_orig
	#img_np = pil_to_np(img_pil)
	#img_np = img_np[None, :]	## Added

	# As we don't have ground truth
	img_pil = img_noisy_pil
	img_np = img_noisy_np

	plt.imshow(img_np[0,:,:,10])
	#plt.imshow(img_noisy_np[:,:,10])
	plt.savefig("images/img_noisy_np_test_4.pdf")
